...
app.MapGet("/api/maintenance/backup-download",
  async (HttpContext http, IServiceProvider sp, ILoggerFactory lf, CancellationToken ct) =>
  {
    ...
    try
    {
      ...
      log.LogInformation("Export complete. Streaming {FileName} ({Size} bytes)", fileName,
    ...
    finally
    {
      _ = Task.Run(async () =>
      {
        ...
        {
          // I don't care if there is an error here
        }
      });
    }
  }).RequireAuthorization("Admin");
