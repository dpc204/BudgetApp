...
public partial class Assign : ComponentBase
{
 // [Inject] private EnvelopeState State { get; set; } = default!;
  ...
  private async Task OnHideChanged(TransactionDto transaction, bool isHidden)
  {
    transaction.TransactionHiddenFromAssign = isHidden;
    await Api.AssignTransactionAsync(
      transaction.TransactionId,
      transaction.LineId,
      transaction.EnvelopeId,
  ...
  private async Task OnShowHiddenToggleChanged(bool showHidden)
  {
