...
public sealed class TransactionsApiClient(HttpClient http, ILogger<TransactionsApiClient> logger) : ITransactionsApiClient
{
  ...
  public async Task<bool> AssignTransactionAsync(int transactionId, int lineId, int envelopeId, string vendor, string description, string notes, bool isHidden = false,
    CancellationToken cancellationToken = default)
  {
    var payload = new
      { TransactionId = transactionId, LineId = lineId, EnvelopeId = envelopeId, Vendor = vendor, Description = description, Notes = notes, TransactionHiddenFromAssign = isHidden };
